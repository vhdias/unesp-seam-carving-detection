...
class SVM:
	...
	@staticmethod
	def input_fn():
		return SVM.column_dict, SVM.class_list

	@staticmethod
	def predict_fn():
		return SVM.column_dict

	def fill_features(self, edited_image_names, not_edited_image_names):
		col = 0
		for image in edited_image_names:
			values = self.edited_data_frame[image].values
		...
		for image in not_edited_image_names:
			values = self.not_edited_data_frame[image].values
			for line in range(self.not_edited_data_frame.shape[0]):
				self.column_dict[SVM.label_feature_list[line][0]][col] = values[line]
			col += 1

		for line in range(len(SVM.label_feature_list)):
			self.column_dict[SVM.label_feature_list[line][0]] = tf.constant(
				self.column_dict[SVM.label_feature_list[line][0]])
	...
	def fit(self):
		self.column_dict = {'example_id': tf.constant(self.list_dict['seam carved']['training'] +
		                                              self.list_dict['untouched']['training'])}
		...
		self.svm.fit(input_fn=SVM.input_fn)

	def evaluate(self):
		self.column_dict = {'example_id': tf.constant(self.list_dict['seam carved']['validation'] +
		                                              self.list_dict['untouched']['validation'])}
